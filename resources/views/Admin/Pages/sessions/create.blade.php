...
@section('main')
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
        ...
        <form class="needs-validation" novalidate action="{{ route('admin.campaigns.sessions.store', $campaign->id) }}" method="POST">
            @csrf
            ...
            <div class="row">
                <div class="col-12 col-lg-4 mb-3">
                    ...
                    <input type="text" class="form-control @error('participant') is-invalid @enderror"
                        id="inputParticipant" name="participant" placeholder="" value="{{ old('participant') }}" />
                    @error('participant')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12 col-lg-4 mb-3">
                    ...
                    <select class="form-control @error('place') is-invalid @enderror" id="selectPlace" name="place">
                        @foreach ($campaign->Places as $place)
                            <option value="{{ $place->id }}" {{ old('place') == $place->id ? "selected" : '' }}>{{ $place->area_and_place }}</option>
                        @endforeach
                    </select>
                    @error('place')
                        <div class="invalid-feedback">{{ $message }}</div>
