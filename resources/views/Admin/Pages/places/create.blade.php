...
@section('main')
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
        ...
        <form class="needs-validation" novalidate action="{{ route('admin.campaigns.places.store', $campaign->id) }}" method="POST">
            @csrf
            <div class="row">
            ...
            <div class="row">
                <div class="col-12 col-lg-4 mb-3">
                    <label for="selectChannel @error('channel') is-invalid @enderror">Area</label>
                    <select class="form-control" id="selectChannel" name="channel">
                        @foreach ($campaign->Areas as $area)
                            <option value="{{ $area->id }}" {{ old('channel') == $area->id ? 'selected' : '' }}>{{ $area->name }} </option>
                        @endforeach
                        @error('channel')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
            ...
            <div class="row">
                <div class="col-12 col-lg-4 mb-3">
                    ...
                    <input type="number" class="form-control @error('capacity') is-invalid @enderror" id="inputCapacity" name="capacity" placeholder=""
                        value="{{ old('capacity') }}">
                    @error('capacity')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
